...
export async function handleNetworkRoutes(path, request, db, url) {
  // AUTH
  ...
  // CIVIC: polls / petitions / proposals (basic endpoints)
  if (path.startsWith("/api/polls"))
    return listPolls(db);

  if (path.startsWith("/api/poll/vote") && request.method === "POST")
    return votePoll(request, db);

  if (path.startsWith("/api/petitions"))
    return listPetitions(db);

  if (path.startsWith("/api/petition/sign") && request.method === "POST")
    return signPetition(request, db);
